...
<html>
  ...
  <body bgcolor="#AAFFEE">
    ...
    <?php
      ...
      $sql = "SELECT item_order.order_id,item.item_name,item.item_price,user.user_name,user.user_phone,user.user_home,item_order.order_status FROM `item_order` INNER JOIN `item` ON item.item_id = item_order.order_product INNER JOIN `user` ON item_order.order_seller = user.user_account WHERE item_order.order_user = '$user'";
